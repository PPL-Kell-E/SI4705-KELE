...
@section('title', 'Tambah Jadwal - MEDISLOT')
@section('page-title', 'Perencanaan Jadwal Pemeriksaan')
@section('page-subtitle', 'Buat jadwal pemeriksaan kesehatan sesuai preferensi Anda')
...
<style>
    ...
    .slot-info {
        margin-top: 8px; font-size: 12px; color: #2d9e72;
        display: none;
    }
    .slot-info.show { display: block; }
</style>
@endsection
...
<div class="form-card">
    <div class="form-card-header">
        <div class="form-card-icon"><i class="fas fa-plus"></i></div>
        <div>
            <div class="form-card-title">Tambah Jadwal Pemeriksaan</div>
            <div class="form-card-subtitle">Buat jadwal pemeriksaan kesehatan sesuai preferensi Anda</div>
        </div>
    </div>
        ...
        <div class="form-body">
            ...
            <div class="form-group">
                ...
                <input type="text" name="fasilitas_klinik"
                       value="{{ old('fasilitas_klinik') }}"
                       placeholder="Contoh: Klinik Medina"
                       required>
                @error('fasilitas_klinik')
                    <div class="form-error">{{ $message }}</div>
                @enderror
            </div>
            ...
            <div class="form-row">
                ...
                <div class="form-group">
                    ...
                    @error('waktu')
                        <div class="form-error">{{ $message }}</div>
                    @enderror
                    <div class="slot-info" id="waktuInfo">
                        <i class="fas fa-check-circle"></i> Waktu tersedia
                    </div>
                </div>
            </div>
            ...
            <div class="form-group">
                ...
                <textarea name="catatan" placeholder='Contoh: "Puasa 10 jam sebelum periksa"'>{{ old('catatan') }}</textarea>
                @error('catatan')
                    <div class="form-error">{{ $message }}</div>
                @enderror
            </div>
...
<script>
    ...
    document.getElementById('inputWaktu').addEventListener('change', function() {
        if (this.value) {
